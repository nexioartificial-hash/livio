...
import { titleCase, formatCUIT } from "@/utils/formatters";
...
export default function ClinicStep({ onNext, onBack }: ClinicStepProps) {
    ...
    const onSubmit = async (values: ClinicFormValues) => {
        ...
        try {
            ...
            // 1b. Create trial subscription (30 days)
            const trialEndsAt = new Date(Date.now() + 30 * 24 * 60 * 60_000).toISOString();
            ...
            if (subError) {
                console.error("❌ [ClinicStep] Error al crear trial:", subError);
                // Non-blocking: trial can be created later, don't stop onboarding
            } else {
                console.log("🎉 [ClinicStep] Trial de 30 días creado (expira:", trialEndsAt, ")");
            }
    ...
    return (
        <div className="h-full flex flex-col p-8 overflow-y-auto custom-scrollbar">
            <div className="space-y-2 mb-8">
                <h2 className="text-3xl font-black text-slate-900 dark:text-white">Datos de tu clínica</h2>
                <p className="text-slate-500 dark:text-slate-400 font-medium">Cuéntanos sobre tu espacio de trabajo.</p>
            </div>

            <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
                {/* Basic Info */}
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6 p-6 rounded-[2rem] bg-slate-50 dark:bg-slate-900/50 border border-slate-100 dark:border-slate-800">
                    <div className="space-y-2">
                        <Label className="text-slate-600 dark:text-slate-400 font-bold ml-1">Nombre Comercial</Label>
                        <div className="relative">
                            ...
                            <Controller
                                ...
                                render={({ field }) => (
                                    <Input
                                        {...field}
                                        className="h-12 pl-12 rounded-xl bg-white dark:bg-slate-950 border-white/50 shadow-sm focus:ring-accent"
                    ...
                    <div className="space-y-2">
                        <Label className="text-slate-600 dark:text-slate-400 font-bold ml-1">Email Profesional</Label>
                        <Input {...register("email")} className="h-12 rounded-xl bg-white dark:bg-slate-950 border-white/50 shadow-sm" placeholder="hola@tunegocio.com" />
                        {errors.email && <p className="text-red-500 text-xs font-bold ml-1">{errors.email.message}</p>}
                    </div>

                    <div className="space-y-2">
                        <Label className="text-slate-600 dark:text-slate-400 font-bold ml-1">Teléfono de Consultas</Label>
                        <Input {...register("phone")} className="h-12 rounded-xl bg-white dark:bg-slate-950 border-white/50 shadow-sm" placeholder="+54 11 ..." />
                        {errors.phone && <p className="text-red-500 text-xs font-bold ml-1">{errors.phone.message}</p>}
                    </div>

                    <div className="space-y-2">
                        <Label className="text-slate-600 dark:text-slate-400 font-bold ml-1">CUIT (Opcional)</Label>
                        <Controller
                            name="cuit"
                            control={control}
                            render={({ field }) => (
                                <Input
                                    {...field}
                                    className="h-12 rounded-xl bg-white dark:bg-slate-950 border-white/50 shadow-sm"
                ...
                <div className="space-y-6">
                    <div className="flex justify-between items-center">
                        <div>
                            <h3 className="text-xl font-bold text-slate-900 dark:text-white">Sucursales ({fields.length})</h3>
                            <p className="text-sm text-slate-500 dark:text-slate-400">¿Tienes más de una ubicación?</p>
                        </div>
                        <Button
                            type="button"
                            variant="outline"
                            onClick={() => append({ name: `Sucursal ${fields.length + 1}`, address: "", google_maps_url: "" })}
                            className="rounded-xl font-bold gap-2 text-accent border-accent/20 hover:bg-accent/5"
                        >
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div className="space-y-2">
                                        ...
                                        <Controller
                                            ...
                                            render={({ field }) => (
                                                <Input
                                                    {...field}
                                                    className="h-11 rounded-xl bg-white dark:bg-slate-950 shadow-sm border-transparent"
                                                    onChange={(e) => field.onChange(titleCase(e.target.value))}
                                                />
                                            )}
                                        />
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <div className="relative">
                                            ...
                                            <Input {...register(`branches.${index}.address`)} className="h-11 pl-9 rounded-xl bg-white dark:bg-slate-950 shadow-sm border-transparent" placeholder="Calle 123, Ciudad" />
                                        </div>
                                    </div>
                                    <div className="md:col-span-2 space-y-2">
                                        <Label className="text-xs font-black uppercase tracking-tighter text-slate-400">Google Maps URL (Embed/Link)</Label>
                                        <Input {...register(`branches.${index}.google_maps_url`)} className="h-11 rounded-xl bg-white dark:bg-slate-950 shadow-sm border-transparent" placeholder="https://maps.google.com/..." />
                                    </div>
                                </div>
                            </motion.div>
                        ))}
                ...
                <div className="flex justify-between pt-8">
                    ...
                    <Button type="submit" disabled={loading} className="h-12 px-10 rounded-xl bg-accent hover:bg-accent/90 text-slate-900 dark:text-white font-black shadow-lg shadow-accent/20 transition-all">
