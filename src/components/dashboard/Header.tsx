            ...
            <div className="flex items-center gap-2 md:gap-4">
                ...
                <DropdownMenu>
                    <DropdownMenuTrigger asChild>
                        <Button variant="ghost" size="icon" className="relative text-slate-500 hover:text-[#76D7B6] transition-colors">
                            <Bell className="h-5 w-5" />
                            {unreadCount > 0 && (
                                <span className="absolute top-1 right-1 flex items-center justify-center min-w-[16px] h-4 px-1 text-[10px] font-bold bg-red-500 text-white rounded-full border-2 border-white leading-none">
                    ...
                    <DropdownMenuContent
                        align="end"
                        className="w-96 p-0 overflow-hidden rounded-2xl shadow-2xl border border-slate-100"
                        sideOffset={8}
                    >
                        {/* Header */}
                        <div className="bg-gradient-to-r from-slate-900 to-slate-800 px-4 py-3 flex items-center justify-between">
                            ...
                            {unreadCount > 0 && (
                                <button
                                    ...
                                    className="flex items-center gap-1 text-xs text-[#76D7B6] hover:text-emerald-300 font-semibold transition-colors"
                                >
                        ...
                        <div className="max-h-[380px] overflow-y-auto divide-y divide-slate-50">
                            {loading ? (
                                <div className="flex items-center justify-center py-10 text-slate-400">
                                    <div className="h-5 w-5 rounded-full border-2 border-[#76D7B6] border-t-transparent animate-spin" />
                                </div>
                                ...
                                <div className="flex flex-col items-center justify-center py-10 gap-2 text-slate-400">
                                    ...
                                    <button
                                        ...
                                        className="text-xs text-[#76D7B6] hover:underline mt-1"
                                    >
                                        Generar demo
                                    </button>
                                </div>
                                ...
                                notifications.map((notif) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={cn(
                                                "w-full flex items-start gap-3 px-4 py-3 text-left hover:bg-slate-50 transition-colors group",
                                        ...
                                        >
                                            ...
                                            <div className="flex-1 min-w-0">
                                                <div className="flex items-center gap-2">
                                                    ...
                                                    {notif.unread && (
                                                        <span className="shrink-0 w-2 h-2 rounded-full bg-[#76D7B6]" />
                                                    )}
                                                </div>
                                                {notif.body && (
