    ...
    return (
        <div className="flex h-screen items-center justify-center bg-slate-50 px-4">
            <div className="w-full max-w-sm bg-white rounded-2xl shadow-lg border border-slate-200 px-8 py-10 text-center">
                ...
                {state === "loading" && (
                    <>
                        <Loader2 className="h-12 w-12 animate-spin text-[#76D7B6] mx-auto mb-4" />
                        <h2 className="text-xl font-bold text-slate-900">Verificando tu email...</h2>
                ...
                {state === "success" && (
                    <>
                        <CheckCircle className="h-14 w-14 text-[#76D7B6] mx-auto mb-4" />
                        <h2 className="text-xl font-bold text-slate-900">¡Tu cuenta está verificada!</h2>
                        <p className="mt-2 text-sm text-slate-500">Ya podés empezar a usar Livio.</p>
                        <Link href="/login">
                            <Button className="w-full mt-6 bg-[#76D7B6] hover:bg-[#65cba8] text-white font-bold">
                                Iniciar Sesión
                            </Button>
                        </Link>
                    </>
                ...
                {state === "already-verified" && (
                    <>
                        <CheckCircle className="h-14 w-14 text-[#76D7B6] mx-auto mb-4" />
                        <h2 className="text-xl font-bold text-slate-900">Tu email ya estaba verificado</h2>
                        <p className="mt-2 text-sm text-slate-500">Podés iniciar sesión normalmente.</p>
                        <Link href="/login">
                ...
                {state === "expired" && (
                    <>
                        ...
                        <h2 className="text-xl font-bold text-slate-900">El link expiró</h2>
                        ...
                        {!resent && (
                            <Button
                                ...
                                disabled={resending}
                                className="w-full mt-6 bg-[#76D7B6] hover:bg-[#65cba8] text-white font-bold"
                            >
                                {resending ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : null}
                                Reenviar email de verificación
                            </Button>
                        )}
                        <Link href="/login" className="block mt-3 text-sm text-slate-500 hover:text-slate-700">
                            Volver al login
                        </Link>
                    </>
                )}
                ...
                {state === "invalid" && (
                    <>
                        ...
                        <h2 className="text-xl font-bold text-slate-900">Link no válido</h2>
                        <p className="mt-2 text-sm text-slate-500">
